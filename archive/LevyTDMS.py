...
class LevyTDMS:
    ...
    def plot_heatmap_1(self, extracted_data, sig_channel, x_channel, y_channel=None, title='Heatmap', xlabel='X-axis', ylabel='Y-axis'):
        ...
        try:
            ...
            plt.figure(figsize=(10, 6))
            plt.imshow(np.abs(sig + 1e-12), aspect='auto', cmap='plasma', origin='lower', extent=extent)
            plt.colorbar(label="Photocurrent [A]")
            plt.xlabel(xlabel)
            plt.ylabel(ylabel)
            plt.title(title)
...
if __name__ == "__main__":
    ...
    files = LevyTDMS(r"G:\.shortcut-targets-by-id\0B8-gGFa6hkR4XzJJMDlqZXVKRk0\ansom\Data\THz 1\SA40653C.20250526\35 - 20250714_TDvSDBias_45mmInsertion_100K_Fine_-100m to 100m Bias_5mSteps")
